...
class Parameterized:
  ...
  def makeProbs(self, e):
    return e/np.sum(e)

  def takeChoice(self, probs):
    return self.rng.choice(len(self.logits), p=probs)

  def __call__(self):
    # https://stackoverflow.com/questions/58339083/how-to-sample-from-a-log-probability-distribution
    e = self.makeExp()
    p = self.makeProbs(e)
    idx = self.takeChoice(p)
...
def timeit(replay, timer, experience_len, n_experience, n_samples_per_step, n_samples_after):
  ...
  timer.wrap('selector', replay.sampler, ['makeExp', 'makeProbs', 'takeChoice'])
...
if __name__ == '__main__':
  ...
  N_SAMPLES_AFTER = 0
  timer = timeit(TestReplay(CAPACITY, seed=SEED), Timer(), EXP_LEN, NUM_TIMES_EXP, N_SAMPLES_PER_STEP, N_SAMPLES_AFTER)
  print('DONE TRAINING\n')
